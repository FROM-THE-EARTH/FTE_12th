...
int main(){
    ...
    */
    ...
    f_mount(&fs,"",0);
    FIL fp;
    results = f_open(&fp,"IZU.CSV",FA_CREATE_ALWAYS|FA_WRITE);
    if(results==0){
        pc.printf("f_open:success");
    }else{
        pc.printf("f_open:false");
        f_open(&fp,"IZU.CSV",FA_CREATE_ALWAYS|FA_WRITE);
    }
     ...
     while(phase!=4){
         ...
         switch (phase){
            case 0:
                ...
                break;
            case 1:
                if(interval()>10 || (maxAltitude-calcMedian(altArray, SAMPLES)>5)){//打ち上がってから15秒後、もしくは10m落下すれば
