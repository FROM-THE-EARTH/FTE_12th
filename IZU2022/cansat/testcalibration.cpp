...
int main(){
    ...
    */
    ...
    float averageMagX, averageMagY;//平均値
    float sumX,sumY;//xとyの総和
    //float sX, sY;
    float dispersionX, dispersionY;//xとyの分散
    float sumdispersionX, sumdispersionY;//(mag - average)^2の総和
    ...
    //平均を求める
    averageMagX = sumX/SAMPLES; 
    averageMagY = sumY/SAMPLES;
    ...
    for(int i=0; i<500; i++){
        for(int j=i+1; j<500; j++){
            if(magArrayY[i] > magArrayY[j]){
                float tmpY = magArrayY[i];
                magArrayY[i] = magArrayY[j];
                magArrayY[j] = tmpY;
            }
        }
    }
