...
#define TARGET_LAT 38.261109 //目標の緯度
#define TARGET_LNG 140.854808 //目標の経度
...
void sendDatas(bool prt){//データを文字列に変換してimSendを呼び出して送信する関数
    ...
    sprintf(sendData,"%d,%d,%.2f,%.2f,%.2f,%f,%f,%.2f,%.2f,%d",
        times, phase, azimuth, angle, direction, thisPos.latitude, thisPos.longtitude, sonicL.distance, sonicR.distance, move);
    wait_us(100);
    imSend(sendData);
    dataNumber++;
}