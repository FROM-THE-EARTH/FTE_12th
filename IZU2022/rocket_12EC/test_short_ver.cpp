...
int main(){
    ...
    while(phase!=3){
        ...
        switch (phase){
            case 0:
                if(flightPin || (acc[0]*acc[0]+acc[1]*acc[1]+acc[2]*acc[2])>=2*2){//フライトピンが抜ける、もしくは2G以上の加速度があれば
                    launched = true;
                    imSend("Launched!!");
                    phase++;
                    //imSend("Phase1 Start");
...
void setUp(){
    ...
    flightPin.mode(PullUp);//フライトピンに電圧をかける
    imSend("Waiting...");
    wait_ms(1000);
    if(flightPin){//この段階でピンが抜けていれば
