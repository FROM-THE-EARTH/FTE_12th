...
int main(){
    ...
    thisPos.longitude = 0;//140.8519829;//THISPOS_LNG;
...
void calcDistance(){//距離計算用関数
    ...
    float y1 = thisPos.longitude*PI/180;
    float x2 = targetPos.latitude*(PI/180);
    float y2 = targetPos.longitude*(PI/180);
...
void calcAngle(){//角度計算用関数 :北0度西90度南180度東270度
    float Y = (cos(targetPos.latitude))*sin(targetPos.longitude - thisPos.longitude);
    float X = (cos(thisPos.longitude))*sin(targetPos.longitude) - (sin(thisPos.latitude))*(cos(targetPos.latitude))*(cos(thisPos.longitude - targetPos.longitude));
